...
Data_Split::Data_Split(string f_name,int max_line){
    init(f_name,max_line);
}

void Data_Split::init(string f_name,int max_line){
    ...
    multi_label.open((SPLIT_DIR+"true_multilabel").c_str());
...
void Data_Split::multi_split(){
    double l;
    string mlabel;
    int index=0;

    while (file_in >> mlabel){
        ...
        while (1){
            ...
            while (rr<mlabel.length() && mlabel[rr] && mlabel[rr]!=',' && mlabel[rr]!=' ')
                rr++;
            l=atof(mlabel.substr(ll,rr-ll).c_str());
            rr++;

            if (label_file.find(l)==label_file.end()){
                char tmp[1000];
                sprintf(tmp,"%lf",l);
                string out_name=SPLIT_DIR+file_name+"_ML"+tmp;
